...
import type { ChatMessage, Conversation, PublicProfile } from "@/lib/types";
...
function MediaBody({ account, conversation, message, onReady }: { account: UnlockedAccount | null; conversation: Conversation; message: ChatMessage; onReady?: () => void }) {
...
export function ChatApp() {
  ...
  useEffect(() => {
    ...
    void createSafetyNumber(
      ...
      const stored = window.localStorage.getItem(`whispr-trust:${account.profile.id}:${activeConversation.person.id}`);
  ...
  async function sendMessage() {
    ...
    const message: ChatMessage = { id, conversationId: active.id, senderId: currentUserId, createdAt: new Date().toISOString(), payload: { kind: "text", text }, pending: Boolean(account) };
    setConversations((items) => items.map((item) => item.id === active.id ? { ...item, messages: [...item.messages, message] } : item));
    setDraft("");
    setShowEmoji(false);
    if (account) {
      try {
        const saved = await sendEncryptedEvent(account, active, { kind: "text", text }, id);
  ...
  async function attachFile(file?: File) {
    ...
    const message: ChatMessage = {
      ...
      payload: { kind: "media", media: { name: file.name, mime: file.type, size: file.size, url: URL.createObjectURL(file) } },
    };
    setConversations((items) => items.map((item) => item.id === active.id ? { ...item, messages: [...item.messages, message] } : item));
    if (account) {
      setUploading(true);
      try {
        const saved = await sendEncryptedMedia(account, active, file, id);
        setConversations((items) => items.map((item) => item.id === active.id ? { ...item, messages: item.messages.map((current) => current.id === id ? saved : current) } : item));
      } catch (reason) {
        setConversations((items) => items.map((item) => item.id === active.id ? { ...item, messages: item.messages.map((current) => current.id === id ? { ...current, pending: false, failed: true } : current) } : item));
  ...
  function trustSafetyNumber() {
    ...
    window.localStorage.setItem(`whispr-trust:${account.profile.id}:${activeConversation.person.id}`, safetyNumber);
  ...
  if (!account && !demoProfile) {
    return <AuthScreen onAuthenticated={(value) => { setConversations([]); setActiveId(""); setAccount(value); setToast("Encryption key unlocked on this device"); }} onDemo={(username) => { setDemoProfile({ ...DEMO_USER, username: username || DEMO_USER.username }); setToast("Demo mode · Connect Supabase to go live"); }} />;
  }
    ...
    <main className={`app-frame ${showMobileChat ? "mobile-chat-open" : "mobile-list-open"}`}>
      <aside className="sidebar">
        <header className="sidebar-head">
          <div className="brand"><span className="brand-mark">w</span><span>whispr</span></div>
        ...
        <div className="search-wrap">
          ...
          <input value={query} onChange={(event) => { setQuery(event.target.value.toLowerCase()); if (event.target.value) setShowPeople(true); }} onFocus={() => query && setShowPeople(true)} aria-label="Search people" placeholder="Search everyone on whispr" />
                ...
                <div className="message-stack">
                  {message.payload.kind === "media" && message.payload.media ? (
                    <div className="bubble media-bubble">
                      <MediaBody account={account} conversation={active} message={message} onReady={keepLatestMediaVisible} />
                    </div>
                  ) : <div className="bubble">{message.payload.text}</div>}
                  <button className="react-trigger" onClick={() => setReactionFor(reactionFor === message.id ? null : message.id)} aria-label="React to message"><Icon name="emoji" /></button>
